...
import { Filter, Eye, Camera, RefreshCw, AlertTriangle, Car, Construction, TrafficCone, Compass } from 'lucide-react';
...
export const TrafficIncidentsView: React.FC<TrafficIncidentsViewProps> = ({
  ...
  const activeIncidentsCount = 24;
  const severeCongestionCount = 5;

  return (
    <div className="flex-1 md:ml-72 flex flex-col lg:flex-row bg-[#f8f9fa] h-[calc(100vh-64px-44px)] overflow-hidden">
      {/* Map Container (Main Left/Center View) */}
      <div
        id="traffic-map-container"
        className="flex-1 relative h-[50vh] lg:h-full min-h-[380px] bg-[#d9dadb] overflow-hidden select-none"
      >
        {/* Background Map Image (Singapore Road Network) */}
        <div
        ...
        >
          ...
          {incidents.map((incident) => {
            ...
            return (
              ...
              >
                ...
                {isSelected && (
                  <div className="absolute bottom-full mb-1 bg-[#191c1d] text-white text-[11px] font-semibold px-2 py-1 rounded shadow-lg whitespace-nowrap animate-in fade-in duration-150">
                    {incident.title}
          ...
          {showCameras && (
            <div
              id="map-marker-cam-pie"
              onClick={() => setActiveIncidentId('inc-1')}
              className="absolute top-[48%] left-[64%] map-marker cursor-pointer z-15 bg-white/95 rounded border border-[#c1c6d3] shadow-md p-1 flex items-center gap-1.5 hover:scale-105 transition-transform"
            >
              <img
                src="https://images.unsplash.com/photo-1545454675-3531b543be5d?auto=format&fit=crop&w=120&q=70"
                alt="PIE Camera Feed"
                className="w-10 h-7 object-cover rounded"
              />
              <div className="text-[9px] font-bold text-[#004481] pr-1 leading-tight">
                CAM 102
                <div className="text-[8px] text-[#727783] font-normal">CTE/PIE</div>
              </div>
            </div>
          )}
        </div>

        {/* Top-Left: NETWORK OVERVIEW Overlay Card matching mockup */}
        <div
        ...
        {/* Map Speed Legend Overlay (like in original mockup top left side) */}
        {showSpeedLegend && (
          <div
            id="traffic-speed-legend"
            className="hidden sm:flex absolute bottom-6 left-4 bg-white/90 backdrop-blur-sm p-3 rounded-lg border border-[#c1c6d3] shadow-sm z-30 flex-col gap-1.5 text-[11px]"
          >
            <div className="font-bold text-[#414751] text-[10px] uppercase tracking-wider mb-1">
              Expressway Status
            </div>
            <div className="flex items-center gap-2">
              <span className="w-3 h-1.5 rounded bg-emerald-500"></span>
              <span className="text-[#414751]">Smooth (&gt;60 km/h)</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="w-3 h-1.5 rounded bg-amber-400"></span>
              <span className="text-[#414751]">Moderate (40-60)</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="w-3 h-1.5 rounded bg-orange-500"></span>
              <span className="text-[#414751]">Slow (20-40)</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="w-3 h-1.5 rounded bg-red-600"></span>
              <span className="text-[#414751]">Very Slow (&lt;20)</span>
            </div>
          </div>
        )}
        ...
        <div className="absolute top-4 right-4 flex flex-col gap-1.5 z-30 bg-white rounded-lg shadow-md border border-[#c1c6d3] p-1">
          <button
            ...
            title="Zoom In"
            className="p-1.5 text-[#414751] hover:bg-[#edeeef] rounded transition-colors text-[13px] font-bold"
          >
            +
          </button>
          <button
        ...
        {activeIncident && (
          ...
          >
            <div className="flex justify-between items-start">
              ...
              <span className="text-[11px] text-[#727783] font-mono">{activeIncident.timeFormatted}</span>
            </div>
            ...
            <div className="flex items-center justify-between pt-1 border-t border-[#edeeef] text-[11px]">
              <span className="text-[#727783]">
                Impact: <strong className="text-[#e51d24]">{activeIncident.laneClosure || 'Lane Affected'}</strong>
              </span>
          ...
          <div className="flex justify-between items-center mb-3">
            ...
            <div className="flex gap-2">
              <button
                id="filter-list-btn"
                title="Filter View"
                className="p-1.5 rounded bg-[#e1e3e4] text-[#414751] hover:bg-[#d9dadb] transition-colors"
              >
