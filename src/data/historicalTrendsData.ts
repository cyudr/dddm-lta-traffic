...
export interface HistoricalTrendsFilterOptions {
...
export function getHistoricalTrendsFallbackData(
  ...
  const dayType = filterOptions?.dayType || 'ALL';
  const incidentType = filterOptions?.incidentType || 'ALL';
  const selectedExp = filterOptions?.expressway || 'ALL';
  ...
  // Count exact occurrences of each weekday in the date range
  ...
  if (timeframe === 'custom' && startDate && endDate) {
    ...
    for (const d of dayNames) {
      if (weekdayCounts[d] === 0) weekdayCounts[d] = 0; // allow 0 if day not in custom range
    }
  }

  // Multipliers based on incidentType filter
  ...
  let baseWeekdayTrends: WeekdayTrendMetric[] = [
    {
      day: 'Monday',
      dayShort: 'Mon',
      isWeekend: false,
  ...
  // 2. Diurnal Hourly Trend Breakdown scaled with rangeDays
  const hourlyTrends = [
  ...
  // 3. Speed Curves: Modulate slightly if weekend vs weekday
  ...
  // 4. Corridor Reliability dynamically computed
  let allCorridors = [
    {
      ...
      status: (dayType === 'WEEKENDS' ? 'On Time' : 'Moderate Delay') as 'On Time' | 'Moderate Delay' | 'Heavy Delay',
    ...
    },
    {
      ...
      status: (dayType === 'WEEKENDS' ? 'On Time' : 'Heavy Delay') as 'On Time' | 'Moderate Delay' | 'Heavy Delay',
  ...
  // 6. Top Bottlenecks with dynamic frequency based on selected date range & filters
  let topBottlenecks = [
